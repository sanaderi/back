            ...
            try
            {
                ...
                return Ok<SchoolResponseViewModel>(new()
                {
                    Data = result.Data is null ? null : new()
                    {
                        ...
                        DefaultImageUri = Url.Action(nameof(FilesController.GetFile), "Files", new { id = result.Data.DefaultImageId, containerType = ContainerType.School }),
            ...
            try
            {
                ...
                return Ok<IEnumerable<SchoolImageInfoViewModel>>(new(result.Errors)
                {
                    Data = result.Data is null ? [] : result.Data.Select(t => new SchoolImageInfoViewModel
                    {
                        ...
                        FileUri = Url.Action(nameof(FilesController.GetFile), "Files", new { id = t.FileId, containerType = ContainerType.School }),
                        TagIcon = t.TagIcon,
                        TagId = t.TagId,
                        TagName = t.TagName,
